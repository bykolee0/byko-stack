...
요청/응답 포맷은 claude-eval과 동일하다 — 같은 파일을 공유할 수 있으며,
...
/codex-eval --cross .myagents/claude-eval-20260326-143052.md                     # claude 결과에 codex 교차검증 추가
```
...
응답 형식:
### [체크리스트 섹션명]
- [PASS|FAIL|WARN] 항목명 — 1줄 근거 (Source Documents의 구체적 내용 인용)

### 전체 판정
- 판정: APPROVED | NEEDS_REVISION
- NEEDS_REVISION인 경우: FAIL 항목별 구체적 수정 제안

각 판정에는 반드시 Source Documents에서 확인한 근거를 포함할 것.
근거 없는 판정은 작성하지 않는다.
</structured_output_contract>

<grounding_rules>
모든 판단은 Source Documents에 실제 작성된 내용에만 기반한다.
문서에 없는 내용을 추측하여 FAIL을 내리지 않는다.
문서의 주장이 코드베이스에 대한 것이면, Target Files를 읽어 사실 여부를 확인한다.
확인할 수 없는 항목은 WARN으로 표기하고 사유를 명시한다.
</grounding_rules>
...
```
# claude 먼저 → codex 교차검증
/claude-eval spec spec.md --output eval-results/spec-claude.md
/codex-eval --cross eval-results/spec-claude.md

# codex 먼저 → claude 교차검증
/codex-eval spec spec.md --output eval-results/spec-codex.md
/claude-eval --cross eval-results/spec-codex.md
```
