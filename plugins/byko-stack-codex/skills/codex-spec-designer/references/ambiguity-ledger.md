...
| `assumed` | AI가 추론, 유저 미확인 | 추론 근거 기재 | **3개 이상이면 게이트 미통과** |
| `open` | 아직 논의되지 않음 | — | **4개 이상이면 게이트 미통과** |
...
- `open` → `assumed`: AI가 추론하되 반드시 `⚠️ 유저 확인 필요` 표시
...
- **신규 개발**: current-state facts, migration이 덜 중요할 수 있음 → `confirmed: "해당 없음"`으로 처리
...
"해당 없음"도 유효한 상태다. 단, AI가 임의로 "해당 없음"을 판단하지 않는다 — 유저에게 확인하고 `confirmed: "해당 없음"`으로 기재한다.
