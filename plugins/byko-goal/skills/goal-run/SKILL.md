...
- 프리플라이트(실패 시 루프 시작 거부):
  ...
  - `run-state.md`에 캡(max_iterations, max_minutes)이 있는가 → 없으면 이 자리에서 한 번 입력받아 기록. 나머지 노브가 없으면 기본값으로 채운다: `per_task_attempt_limit` 3 · `checkpoint_every` 4 · `major_changes_budget` 3 · `stall_limit` 3. 카운터(`last_tick`·`stall_streak`·`dispatched_at`·`last_checkpoint`)가 없으면 추가.
...
반환 없음/턴 소진        → 행의 디스패치 < per_task_attempt_limit 이면 같은 task로 새 worker(이어서 모드) · stall_streak++
                          아니면 BLOCKED:no-progress
