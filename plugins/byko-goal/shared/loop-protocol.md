...
1. 재독 + tick: run-state.md·goal.md(체크리스트) 재독.
   elapsed_minutes += now − last_tick ; last_tick = now
2. 캡:      iterations_used ≥ max_iterations  OR  elapsed_minutes ≥ max_minutes → 정지·인계
3. 공회전:  stall_streak ≥ stall_limit → 정지·보고 (recovery.md)
4. 완료:    체크리스트 전부 [x] → ▶ auditor 최종 패스(체크포인트를 겸한다 — 따로 또 띄우지 않는다)
            → 최종 eval → eval/final-<ts>.md · status=complete → 보고·정지
5. 체크포인트: done_since_checkpoint ≥ checkpoint_every  OR  checkpoint_requested ≠ —
            → ▶ auditor dispatch (goal_dir + last_checkpoint + 사유)
            → concise 델타 적용 (재오픈·체크리스트·문서 정리) → done_since_checkpoint=0, checkpoint_requested=— → 1로
6. 선택:    다음 [ ] task (의존 풀린 것 우선). goal.md [~], run-state: current_task=NN,
            dispatched_at=now, 행의 디스패치++
7. ▶ worker dispatch (byko-goal:worker). goal_dir + task ID만.
8. 결과 수신 → 검증 게이트(아래) → goal.md·run-state 갱신 → iterations_used++ → 1로
```
...
c. 작업 수행. 팬아웃은 조사(explorer)·독립 구현 조각(일반 서브에이전트)에만.
   ...
   시간(per_dispatch_minutes를 정한 목표만): 긴 활동 전에 `now − dispatched_at`을 본다. 예산이 다 되면 — 산출물이 검증 가능한
   상태면 d로, 아니면 진행 상태를 tasks/NN.md 작업 노트에 남기고 "PARTIAL:time-budget" 반환(다음 dispatch가 이어서 한다).
...
worker의 말은 주장이다. 반환의 **첫 줄만** 해석하고 나머지(결론·권고·다음 task에 대한 의견)는 읽지 않는다 — 드라이버 컨텍스트와 독립성을 지킨다. 한 줄 명령으로 확인하고 처리한다:

| worker 반환 | 드라이버 확인 (한 줄 명령) | 처리 |
|---|---|---|
| `DONE` | 최신 회차 파일의 판정: `grep -E '^## 판정:' "$(ls -t eval/NN-[0-9]*.md \| head -1)" \| tail -1` (파일에 판정 줄이 둘이면 마지막 것) | `APPROVED`면 goal.md [x] · run-state done · done_since_checkpoint++ · stall_streak=0. **아니면 거부** — 행 결과에 `unverified-done`, BLOCKED와 같이 처리 |
...
1. **양** — 3~8개. 10개를 넘으면 task가 크거나 조건이 과분할된 신호다. evaluator는 30턴이라 조건마다 명령 하나면 못 끝난다.
...
4. **한 dispatch는 유한하다.** 1~3을 지키면 dispatch는 worker의 턴 한도(도구 단위) 안에서 끝나고, 턴이 다하면 드라이버가 디스패치 한도까지 이어서 띄운 뒤 auditor가 나눈다. 시간으로도 경계를 두고 싶으면 유저가 `per_dispatch_minutes`(§캡, 선택)를 정한다 — 플러그인은 시간값을 정하지 않는다(얼마나 걸리는 일인지는 목표마다 다르다). 정했다면 예산이 다 될 때 검증 가능한 상태면 evaluator로, 아니면 `PARTIAL:time-budget`으로 반환해 다음 dispatch가 잇는다.
...
**드라이버의 델타 적용 (가벼움 유지).**
- **재오픈**: 회귀한 task를 goal.md `[ ]`로 되돌리고, 그 task의 회차 파일을 `archive/eval/`로 옮긴다(새 라운드 — 디스패치·시도 0). history.md에 한 줄.
- **체크리스트 변경**: 추가/분할/정제는 **자율 적용**. 파괴적 변경(삭제/재범위/순서 뒤집기)은 **major로 카운트** — `major_changes_used`가 `major_changes_budget`(기본 3)을 넘으면 정지+사람 호출(목표 표류 방지). 적용한 변경은 history.md에 한 줄.
...
| `per_dispatch_minutes` | **선택 · 기본값 없음.** worker 한 번의 활성 시간 상한. 횟수 캡은 dispatch가 끝나야 세지만 이건 dispatch **안**을 막는다 — 드라이버가 못 보는 구간. 값은 사람이 정한다: 모르면 비워 두고, 첫 체크포인트 뒤 상태표의 분 칸(이 목표의 실측)을 보고 정한다 | 정했다면 worker가 `dispatched_at`을 읽어 스스로 지킨다(§검증의 경계 4). 드라이버는 반환 시 Agent 소요로 대조해 넘긴 dispatch를 `over-time`으로 표시. 환경이 백그라운드 dispatch·정지를 지원하면 예산의 2배를 넘겨도 반환이 없을 때 끊고 이어서 모드로 재디스패치(recovery.md). 비어 있으면 worker maxTurns·디스패치 한도가 상한 |
...
| 역할 | 매 회 읽는 것 | 쓰는 것(소유) |
|---|---|---|
| 드라이버 | run-state.md · goal.md 체크리스트 | run-state.md 전체 · goal.md 체크리스트(상태·auditor 델타) · history.md |
...
| auditor | goal.md · run-state.md · ownership.md · knowledge.md · 델타 구간의 tasks/·handoffs/ | audit/ · knowledge.md(정리) · ownership.md(정리·보정) · history.md · archive/ |

worker는 run-state.md·goal.md를 쓰지 않는다(캡 값·자기 항목을 읽기만). evaluator는 knowledge.md·handoffs·run-state.md를 열지 않는다 — 판정에 꼭 필요하면 조건이 가리키는 파일 하나만.
...
**디스패치**: 설치 환경에선 `Agent`로 `byko-goal:evaluator`를 이름으로 띄운다. 이름이 해석되지 않는 환경(미설치·CI·도그푸딩)에선 일반 서브에이전트를 띄우되 `agents/evaluator.md`를 읽고 따르게 하고 입력은 동일하게 포인터(`goal_dir` + `task ID`)만 준다 — 독립성은 디스패치 방식과 무관하게 유지된다. worker의 explorer 위임, 드라이버의 worker·auditor 디스패치도 같은 폴백을 쓴다.

## 완료

체크리스트가 전부 [x]면 드라이버는 auditor 최종 패스(체크포인트 겸) 뒤 완수조건을 점검한다. 결과는 반드시 디스크에 남는다: 결과/동작 기준이면 evaluator(`mode: final`)가 `eval/final-<ts>.md`를 쓰고, 실행 명령이면 드라이버가 명령·종료코드·출력 요약을 같은 이름의 파일에 `## 판정:` 줄과 함께 쓴다. 통과하면 run-state `status: complete` + history.md `완료`. 재실행(`/goal-run <slug>`)은 `status: complete`를 보고 현황만 보고한다 — 최종 패스를 다시 돌리지 않는다(사람이 재개를 원하면 status를 running으로 되돌린다).

## 크래시·예외

세션이 중간에 죽으면 `run-state.current_task`에 진행중 task가 남는다. 재개 시 드라이버는 `last_tick`을 리셋하고, 해당 task의 회차·디스패치 수를 디스크에서 대조한 뒤(한도 안이면) 새 worker를 "이어서 모드"로 띄운다 — worker가 산출물·tasks/NN.md·eval/ 상태를 보고 이어서 할지/처음부터 할지 판단한다(기본: 이어서). 진전이 전혀 없던 dispatch(파일 변경·회차 0)도 디스패치로 센다. 상세는 `skills/goal-run/references/recovery.md`.
